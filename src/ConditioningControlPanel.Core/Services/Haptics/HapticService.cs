...
namespace ConditioningControlPanel.Core.Services
{
    ...
    public class HapticService : IDisposable
    {
        ...
        public async Task<HapticTestResult> TestAsync()
        {
            ...
            {
                Log.Warning("TestAsync: Not connected - provider={Provider}, connected={Connected}",
            ...
            if (!reachable)
            {
                Log.Warning("TestAsync: Device unreachable on {Provider} - likely VPN/network change", provider.Name);
                await provider.DisconnectAsync();
                Error?.Invoke(this, "Device unreachable");
                return HapticTestResult.Unreachable;
            }

            Log.Information("TestAsync: Starting test pattern on {Provider}", provider.Name);
        ...
        public async Task SetSyncIntensityAsync(double intensity)
        {
            ...
            if (_activeProvider == null || !_activeProvider.IsConnected)
            {
                Log.Debug("SetSyncIntensity: No active provider or not connected");
                return;
            }
            ...
            Log.Information("SetSyncIntensity: Sending {Intensity:F2} to {Provider}", clampedIntensity, _activeProvider.Name);
        ...
        public async Task SetSyncPatternAsync(float[] intensities, int totalDurationMs)
        {
            ...
            if (_activeProvider == null || !_activeProvider.IsConnected)
            {
                Log.Debug("SetSyncPattern: No active provider or not connected");
                return;
            }
