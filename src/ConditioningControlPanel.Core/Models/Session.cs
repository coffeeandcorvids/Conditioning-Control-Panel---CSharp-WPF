...
namespace ConditioningControlPanel.Core.Models
{
    ...
    {
        // Host hooks replacing upstream's App.* service locators (WPF App class).
        // The shell wires these once at startup; unwired they degrade gracefully.
        public static Func<string, string>? ModAwareText { get; set; }
        public static Func<IEnumerable<string>?>? LockCardPhrasesProvider { get; set; }
        ...
        {
            return ModAwareText?.Invoke(Name) ?? Name;
        }
        ...
        {
            return ModAwareText?.Invoke(Description) ?? Description;
        }
...
Your only purpose is to sit prettily and let the pink fog consume you. And remember not to touch that clitty, Good Girls Don't Cum.",
        ...
        public string GetSpoilerInteractive()
        {
            ...
            if (Settings.LockCardEnabled)
            {
                var phrases = LockCardPhrasesProvider?.Invoke();
